\documentclass[10pt]{article}

\usepackage{hyperref} % it needs to come before abntex2cite
\hypersetup{
    colorlinks=true,
    urlcolor=blue,
    linkcolor=blue,
    citecolor=blue,
    pdftitle={Lifeline or trap? The consequences of "vibe coding" and excessive reliance on AI},
    pdfauthor={Krémer Aliz, Hősi Rezső Rudolf, Oláh Sára},
    pdfkeywords={Vibe coding; Debugging Paradox; Programming education; LLMs, Cognitive offloading, Over-reliance}
}

\title{\textbf{ Lifeline or trap? The consequences of "vibe coding" and excessive reliance on AI }}

\author{
    \fontsize{11}{13}\selectfont 
    1\ts{st} Author's Name \\
    \fontsize{10}{11}\selectfont 
    Krémer Aliz\\
    \fontsize{10}{11}\selectfont 
    ELTE\\
    \fontsize{10}{11}\selectfont
    a0tyh8@inf.elte.hu\\
    \and
    \fontsize{11}{13}\selectfont 
    2\ts{nd} Author's Name\\
    \fontsize{10}{11}
    \selectfont Hősi Rezső Rudolf\\
    \fontsize{10}{11}\selectfont 
    ELTE\\
    \fontsize{10}{11}\selectfont
    dov7lk@inf.elte.hu\\
     \and
    \fontsize{11}{13}\selectfont 
    3\ts{nd} Author's Name\\
    \fontsize{10}{11}
    \selectfont Oláh Sára\\
    \fontsize{10}{11}\selectfont 
   ELTE\\
    \fontsize{10}{11}\selectfont
    a4mrc6@inf.elte.hu\\
}

\date{}

%%%%%%%%%%%%%%%%%
% import packages %
%%%%%%%%%%%%%%%%%
\usepackage{lipsum}
\usepackage{lmodern} % customize author's fontsize

% setting page settings and layout 
% https://texdoc.org/serve/geometry/0
\usepackage[
layout=letterpaper, 
paper=letterpaper, 
portrait, 
head=0.5in,
foot=0.5in,
top=1in, 
bottom=1in, 
left=0.75in, 
right=0.75in
]{geometry}

\usepackage[english]{babel}


\setlength{\columnsep}{0.25in} % space/gap between columns
\pagenumbering{gobble}  % supress page number

\usepackage{xurl}
\usepackage[hyperpageref]{backref}	 %  link to bib citations
\usepackage[runin]{abstract}

% https://ctan.dcc.uchile.cl/macros/latex/contrib/abntex2/doc/abntex2cite.pdf pg. 7

\usepackage{etoolbox}
\usepackage{indentfirst} % ident section's paragraphs
\def\ni{\noindent} % remove abstract pragraph

\usepackage{custom_commands} % custom commands, including abstract text

\usepackage{subfiles} % best loaded last in the preamble
\usepackage[pdftex]{graphicx} % add images


% multiple bibliografy files
% https://www.overleaf.com/learn/latex/Questions/Creating_multiple_bibliographies_in_the_same_document
\usepackage[resetlabels,labeled]{multibib}
\newcites{LSR}{Literature Systematic Reviews Reading}
\newcites{CC}{Computer Science Readings}

\begin{document}
\renewcommand{\abstractname}{} % remove 'Abstract' title


\maketitle % create title and authors


%%%%%%%%%%%%
% Abstract %
%%%%%%%%%%%%

\setlength{\absleftindent}{0em}
\setlength{\absrightindent}{0em}

\begin{abstract}
    \subfile{sections/abstract}
\end{abstract}

%%%%%%%%%%%
% Article %
%%%%%%%%%%%

%%%%%%%%%%%
% Bevezetés %
%%%%%%%%%%%
\section{INTRODUCTION}
\subfile{sections/introduction}

%%%%%%%%%%%
% Szakirodalmi áttekintés (Literature Review) %
%%%%%%%%%%%
\section{LITERATURE REVIEW}
\subfile{sections/literature-review}

%%%%%%%%%%%
% Módszertan (Methodology) %
%%%%%%%%%%%
\section{METHODOLOGY}
\subfile{sections/methodology}

%%%%%%%%%%%
% Eredmények és Hipotézisek elemzése (Results) %
%%%%%%%%%%%
\section{RESULTS}
\subfile{sections/result}

%%%%%%%%%%%
% Diszkusszió (Discussion) %
%%%%%%%%%%%
\section{DISCUSSION}
\subfile{sections/discussion}

%%%%%%%%%%%
% Konklúzió: A „Debugolási Paradoxon” %
%%%%%%%%%%%
\section{CONCLUSION}
\subfile{sections/conclusion}


%%%%%%%%%%%%%%
% References
%%%%%%%%%%%%%%
\bibliographystyle{apalike}
\bibliography{refs}

\end{document}